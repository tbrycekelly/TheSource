...
make.proj = function(projection = NULL, lat = NULL, lon = NULL, h = NULL, dlat = 10) {
  ...
  if (!projection %in% projections) {stop('Invalid projection type, please provide one of the following:', paste(projections))}
...
#' @export
add.map.layer = function(lon, lat, z, zlim = NULL, pal = 'ocean.algae', col.na = NA, n = 255, refinement = 0,
  ...
  if (refinement > 0) {
    for (i in 1:refinement) {
      ...
      z = oce::bilinearInterp(grid$lon, grid$lat, lon.old, lat.old, z)
...
test.map.california = function() {
    ...
    map = make.map(coast = "coastlineWorldFine", p = "+proj=merc",
             ...
             dlat = 5, dlon = 5)
    add.map.bathy.shade(map, bathy.pacific, refinement = 1)
    map
}
